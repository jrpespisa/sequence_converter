...
feature "biologist visits index page" do
  ...
  scenario "biologist inputs dna sequence that contains invalid nucleotides", js: true do
    ...
    expect(page).to have_content "Please only include valid DNA sequence (A, C, T, G)."
    expect(page).to_not have_content "ugugugffa"
    expect(page).to_not have_content "CVffa"
  end
